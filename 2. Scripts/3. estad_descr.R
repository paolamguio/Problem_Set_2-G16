...
prop.table(table(df_test_hogares$Pobre)) #20% pobres y 80% no pobre

## Se seleccionan variables de interes
dftest <- df_test_hogares %>% select(c("Pobre", "Nper", "Nro_personas_trabajo_formal", "tipo_vivienda", "Dominio", "Nro_personas_cuartos", "cuota_amortizacion", "arriendo", "edad_promedio", "jefe_hogar_mujer", "Nro_hijos", "edu_promedio", "horas_trabajadas_promedio", "porcentaje_mujeres", "porcentaje_trabajo_formal", "porcentaje_subsidio_familiar", "segundo_trabajo", "otros_ingresos", "otros_ingresos_instituciones", "tasa_ocupacion", "tasa_desempleo", "tasa_participacion", "Ingtotob_hogar"))
df2test <- dftest %>% mutate(loging = log(Ingtotob_hogar))
...
descriptivastest$Estadisticas <- row.names(descriptivas) # se crea columna dentro del dataframe con el nombre de las filas 
descriptivastest <- descriptivas %>% select(Estadisticas, everything()) # se ubica la columna creada en la primera posición 
...
tbl_summary(dftest, by= Pobre, statistic = list (all_continuous()~"{mean} ({sd})")) # por clasificación

## Gráficos

#Gráfico dispersión por ingreso
ggplot(data = df2test, mapping = aes(x = Pobre , y = log)) +
  labs(x = "Pobre", y = "Ingreso promedio hogar") +
  geom_point(col = "chocolate4" , size = 0.5) 

ggplot(data = dftest , mapping = aes(x = Pobre , y = Nro_personas_cuartos)) +
  geom_point(col = "red" , size = 0.5) 
